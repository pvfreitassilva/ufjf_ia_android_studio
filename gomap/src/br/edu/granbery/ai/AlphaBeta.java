	...
	*/
	...
	/*Runnable calculateWithRecursion = new Runnable() {
		public void run() {
			...
			Log.d("Testando", "Tempo recursão: "+(System.currentTimeMillis()-timeInit));
		}
	};*/
	...
	*/
	...
	private int calculateWithRecursion(Board board, int depth, int alpha, int beta) {
		if (depth == 0 || board.isGameOver()) {
			...
			/*
			 * VER PORQUE PROFUNDIDADE IMPAR INVERTE A SOLUCAO DO ALPHA BETA
			 */
	...
	public Piece getBestMove(Board board) {
		...
		Log.d("Testando","Recursão terminada");
		...
		*/
		...
		Log.d("Testando", "Cortes pilha: " + cutsStack + " Cortes recursão: "
				+ cutsRecursion);
		Log.d("Testando", "Empilhamentos: " + stacking + " Recursões: "
				+ recursions);
				*/
		
		/*
		Log.d("Testando", "Tempo pilha: " + (time2 - time1)
				+ " Tempo recursão: " + (time1 - timeInit));
